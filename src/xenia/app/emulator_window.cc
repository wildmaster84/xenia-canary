  ...
  switch (mode) {
    case xe::kernel::NETWORK_MODE::OFFLINE: {
      emulator_->kernel_state()->BroadcastNotification(
          kXNotificationIDLiveConnectionChanged, X_ONLINE_S_LOGON_DISCONNECTED);

      emulator_->kernel_state()->BroadcastNotification(
          kXNotificationIDLiveLinkStateChanged, 0);
...
void EmulatorWindow::NetplayStatus() {
  ...
  if (xe::kernel::XLiveAPI::interface_name.empty()) {
    ...
    std::string interface_format = "Network Interface: {}";

    if (xe::kernel::XLiveAPI::GetInitState() !=
            xe::kernel::XLiveAPI::InitState::Pending &&
        cvars::network_mode != xe::kernel::NETWORK_MODE::OFFLINE) {
      interface_format = "Network Interface: {} - {}";
    }

    msg += fmt::format(interface_format, xe::kernel::XLiveAPI::interface_name,
                       WAN_interface);
  }
