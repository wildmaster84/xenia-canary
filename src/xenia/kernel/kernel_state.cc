...
object_ref<XThread> KernelState::LaunchModule(object_ref<UserModule> module) {
  ...
  // TODO(gibbed): terrible hack
  if (module->name() == "bootanim" || module->name() == "$flash_bootanim") {
  ...
  if (!start_address) {
    XELOGW("KernelState: Wanted to launch module, but has no start address!");
    return nullptr;
  }
...
void KernelState::RegisterNotifyListener(XNotifyListener* listener) {
  ...
  if (!has_notified_startup_ && listener->mask() & kXNotifySystem) {
    ...
    listener->EnqueueNotification(kXNotificationTrayStateChanged,
                                  kXNotificationTrayStateClosed);
    listener->EnqueueNotification(kXNotificationIDSystemStorageDevicesChanged,
                                  0);
    listener->EnqueueNotification(kXNotificationIDSystemStorageDevicesChanged,
                                  1);
    listener->EnqueueNotification(kXNotificationIDLivePresenceChanged, 1);
  }
